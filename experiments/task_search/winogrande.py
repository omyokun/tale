...
df = pd.read_csv(os.environ.get("TALE_DATA_PATH", "data/winogrande/winogrande_debiased_validation.csv"))
print(f"Dataset length: {len(df)}")

tokenizer = AutoTokenizer.from_pretrained(os.environ.get("TALE_MODEL_PATH", "meta-llama/Llama-3.1-8B-Instruct"))
model_base = AutoModelForCausalLM.from_pretrained(os.environ.get("TALE_MODEL_PATH", "meta-llama/Llama-3.1-8B-Instruct"))
...
def role(question):    
    """5-shot system prompt for Winogrande - pronoun resolution and commonsense reasoning"""
    chat = [
        {
            "role": "system",
            "content": (
                "You are a helpful assistant that solves pronoun resolution tasks. "
                "You will be given a sentence with a blank (_) and two options to fill in the blank. "
                "Choose the option that makes the most logical sense based on commonsense reasoning and context. "
                "Respond with only the number of your chosen option (1 or 2)."
            ),
        },
...
def format_winogrande_question(sentence, option1, option2):
    """Format Winogrande question with sentence and two options"""
    formatted_question = f"Sentence: {sentence}\n"
    formatted_question += f"Option 1: {option1}\n"
    formatted_question += f"Option 2: {option2}\n"
    formatted_question += "Which option should fill in the blank (_)?"
    return formatted_question

def evaluate_model(model_instance, num_layers=None, model_name="Original", max_questions=None):
    """Evaluate model performance on Winogrande dataset"""
    correct = 0
    total = 0
    predictions = []
    ...
    num_to_process = min(max_questions or NUM_QUESTIONS, NUM_QUESTIONS)
    ...
    for i in tqdm(range(num_to_process), desc=f"Processing {model_name}"):
        ...
        formatted_question = format_winogrande_question(sentence, option1, option2)
        ...
        with torch.no_grad():
            ...
            if len(predicted_text) > 0:
                predicted_answer = predicted_text.strip()
                if predicted_answer not in ['1', '2']:
                    if '1' in predicted_answer:
                        predicted_answer = '1'
                    elif '2' in predicted_answer:
                        predicted_answer = '2'
                    else:
    ...
    for i in range(min(10, len(predictions))):
        status = "✓" if predictions[i] == answers[i] else "✗"
        print(f"  {i+1:2d}. Predicted: {predictions[i]}, Actual: {answers[i]} {status}")
    
    return accuracy, predictions

def test_different_layer_counts(max_questions=550):
    ...
    layer_counts_to_test = [i+1 for i in range(original_layer_count)]  
    
    results = {}
    
    for num_layers in layer_counts_to_test:
        if num_layers <= original_layer_count:
            print(f"\nCreating modified model with {num_layers} layers...")
            modified_model = ModifiedLlamaModel(model_base, num_layers)
            modified_model.eval()
