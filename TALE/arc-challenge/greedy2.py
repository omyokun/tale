...
model_base = AutoModelForCausalLM.from_pretrained("/work/m24047/m24047nmmr/llama-3.1-8b-instruct/Llama-8B-ARC-PRUNED_64bothbis")
...
def extract_answer_letter(generated_text):
    ...
    # If text is empty, return INVALID
    if not text:
        return "INVALID"
    
    # Check for pattern like "(A)" or "(A) some text"
    if text.startswith('(') and len(text) >= 3:
        potential_letter = text[1].upper()
        if potential_letter in ['A', 'B', 'C', 'D'] and text[2] == ')':
            return potential_letter
    
    # Check if it starts directly with a letter (A, B, C, D)
    if len(text) >= 1 and text[0].upper() in ['A', 'B', 'C', 'D']:
        return text[0].upper()
    
    # Look for the first letter that's A, B, C, or D as fallback
    for char in text:
        if char.upper() in ['A', 'B', 'C', 'D']:
            return char.upper()
    
    # If no valid letter found, return INVALID
    return "INVALID"
...
def evaluate_model(model_instance, model_name="Model"):
    ...
    for i in tqdm(range(290), desc=f"Processing {model_name}"):
        ...
        with torch.no_grad():
            ...
            predicted_answer = extract_answer_letter(predicted_text)
