...
df = pd.read_csv("/tmpdir/m24047nmmr/pruning/datasets/commonsense_qa/commonsenseqa_validation.csv")
...
model_base = LlamaForCausalLM.from_pretrained("/work/m24047/m24047nmmr/llama-3.1-8b-instruct/Llama-8B-commonqa-PRUNED_64bothbis")
# tokenizer = AutoTokenizer.from_pretrained("/work/m24047/m24047nmmr/lucie-7b-instruct-v11")
# model_base = AutoModelForCausalLM.from_pretrained("/work/m24047/m24047nmmr/lucie-7b-instruct-v11")
...
def role(question):    
    """5-shot system prompt for CommonsenseQA - focused on commonsense reasoning"""
    chat = [
        {
            "role": "system",
            "content": (
                "You are a helpful assistant that answers multiple-choice questions requiring commonsense knowledge and reasoning. "
                "Read each question carefully and select the most logical answer from the given options based on common knowledge and reasoning. "
                "Respond with only the letter of your chosen answer (A, B, C, D, or E)."
            ),
        },
...
for _, row in df.iterrows():
    ...
    # Build choices list - CommonsenseQA has 5 choices (A, B, C, D, E)
    choices = []
    for choice_letter in ['A', 'B', 'C', 'D', 'E']:
        choice_text = row[f'choice_{choice_letter}']
        if pd.notna(choice_text) and choice_text.strip():  # Only add non-empty choices
...
def evaluate_model(model_instance, num_layers=None, model_name="Original"):
    """Evaluate model performance on CommonsenseQA dataset"""
    correct = 0
    total = 0
    predictions = []
    concept_results = {}
    ...
    num_to_process = min(NUM_QUESTIONS, 1000)
    
    for i in tqdm(range(num_to_process), desc=f"Processing {model_name}"):
        question = question_list[i]
        choices = choices_by_question[i] 
        answer_key = answer_keys[i]
        question_concept = question_concepts[i]
        
        # Track concept-wise performance
        if question_concept not in concept_results:
            concept_results[question_concept] = {'correct': 0, 'total': 0}
        ...
        with torch.no_grad():
            ...
            if len(predicted_text) > 0:
                ...
                # Ensure it's a valid choice (A, B, C, D, E)
            ...
            if predicted_answer == answer_key:
                correct += 1
                concept_results[question_concept]['correct'] += 1
            
            total += 1
            concept_results[question_concept]['total'] += 1
            
    accuracy = correct / total if total > 0 else 0
    print(f"{model_name} Accuracy: {accuracy:.4f} ({correct}/{total})")
    
    # Print concept-wise results (top 15 concepts by count)
    print(f"\nTop 15 Concept-wise results for {model_name}:")
    sorted_concepts = sorted(concept_results.items(), key=lambda x: x[1]['total'], reverse=True)[:15]
    for concept, results in sorted_concepts:
        concept_acc = results['correct'] / results['total'] if results['total'] > 0 else 0
        print(f"  {concept:20}: {concept_acc:.3f} ({results['correct']}/{results['total']})")
...
def get_baseline_accuracy(return_model=False):
    ...
    #baseline_model = ModifiedLlamaModel(model_base, delete_indices=set([18,22,27]))
